...
class Info_revisor_model extends CI_Model {
    ...
    function lista_revisores_por_tema($tema)
    {
        ...
        (SELECT  COUNT(1) as cantidad FROM  revista as r WHERE  (rev.ID = r.id_revisor_1 or rev.ID = r.id_revisor_2 or rev.ID = r.id_revisor_3) AND r.id_estado=3 ) as cantidad 
        FROM revisor as rev WHERE rev.ID != 0 AND rev.fk_tema = ? ORDER by cantidad ASC", array($tema));
        if ($query) {
            return $query;
        } else {
            return false;
